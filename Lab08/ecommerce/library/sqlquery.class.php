...
class SQLQuery {
    ...
    function connect($address, $account, $pwd, $name) {
		$this->_dbConnect = mysqli_connect($address, $account, $pwd, 'ecommerce');
        $this->_dbHandle = @mysqli_connect($address, $account, $pwd, 'ecommerce');
        if ($this->_dbHandle != 0) {
	...
	function search() {
		...
		#echo '<!--'.$this->_query.'-->';
		$this->_result = mysqli_query($this->_query, $this->_dbHandle);
		$result = array();
		$table = array();
		$field = array();
		$tempResults = array();
			...
			while ($row = mysqli_fetch_row($this->_result)) {
				...
				if ($this->_hM == 1 && isset($this->hasMany)) {
					foreach ($this->hasMany as $aliasChild => $modelChild) {
						...
						#echo '<!--'.$queryChild.'-->';
						$resultChild = mysqli_query($queryChild, $this->_dbHandle);
	...
	protected function _describe() {
		...
		if (!$this->_describe) {
			...
			$this->_result = mysqli_query($this->_dbConnect, $query, $this->_dbHandle);
			while ($row = mysqli_fetch_row($this->_result)) {
